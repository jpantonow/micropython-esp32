...
mq135_feed = bytes('{:s}/feeds/{:s}'.format(ADAFRUIT_USERNAME, MQ135_FEED_ID), 'utf-8') # format - techiesms/feeds/temp
...
def sens_data(data):
    ...
    client.publish(temp_feed, bytes(str(temp), 'utf-8'),qos=0)   # Publishing Temprature to adafruit.io
    
    client.publish(hum_feed,bytes(str(hum), 'utf-8'),qos=0)   # Publishing Temprature to adafruit.io
    print("Temp - ", str(temp))
    print("Hum - " , str(hum))
    print("Ppm - ", str(ppm))
    print('Msg sent')
